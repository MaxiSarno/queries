...
export async function getPendingOrders(db: Database, thresholdDays?: number): Promise<any[]> {
  const thresholdClause = thresholdDays !== undefined
    ? `AND julianday('now') - julianday(o.order_date) > ${thresholdDays}`
    : "";
  const query = `
    SELECT
        o.order_id,
